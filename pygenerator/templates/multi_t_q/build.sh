...
sed -i -e "s/__BUILD_VERSION__/$VERSION/" $SCRATCH_DIR/bin/multi_t_q.sh
...
# rpm包用于线下/线上的标准化部署
fpm -s dir -t rpm -n $PNAME -v $VERSION --rpm-defattrfile=0755 --prefix=/usr/local/mako/prog.d $SCRATCH_DIR
