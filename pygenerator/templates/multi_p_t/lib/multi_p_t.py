...
if __name__ == '__main__':
    ...
    ap.add_argument('-d', '--execute_dir', type=str,
                    ...
                    default=basepath)
    args = ap.parse_args()
    print 'Run multi_p_t service at %s' % args.execute_dir
    ...
    logging.config.fileConfig(os.path.join(args.execute_dir, 'conf/multi_p_t_logging.cfg'))
    # multi_p_t service config
    print 'Load multi_p_t service config...'
    cfg = ConfigParser.RawConfigParser()
    cfg.read(os.path.join(args.execute_dir, 'conf/multi_p_t_service.cfg'))

    # Let's rock 'n roll!
    Multi_p_tService(cfg, args.execute_dir).run()
